...
pub(crate) fn sqlite_err(e: rusqlite::Error) -> crate::api::TheseusSerializableError {
...
pub fn backup_if_due(conn: &Connection, db_path: &Path) -> Result<()> {
	...
	let backup_path = parent.join(format!("chat_backup_{}.db", modified_date.format("%Y%m%d")));
