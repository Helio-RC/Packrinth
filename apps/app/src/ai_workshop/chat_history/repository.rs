...
impl ChatHistoryRepository {
	...
	pub async fn add_message(&self, msg: models::NewMessage) -> Result<models::Message> {
		let now = Utc::now().timestamp();
	...
	pub async fn export_conversation(&self, id: &str, format: &str) -> Result<String> {
		let Some((conversation, messages)) = self.get_conversation(id, u32::MAX, 0).await? else {
		...
		match format {
			"json" => {
				...
				serde_json::to_string_pretty(&payload).map_err(|e| other_err(e.to_string()))
			}
			"markdown" => {
				let mut out = format!("# {}\n\n", conversation.title);
				for message in &messages {
					...
					if let Some(tool_calls) = &message.tool_calls {
						out.push_str(&format!("  - tool_calls: {tool_calls}\n"));
					}
				}
				Ok(out)
			}
			other => Err(other_err(format!("Unsupported export format: {other}"))),
		}
	}
