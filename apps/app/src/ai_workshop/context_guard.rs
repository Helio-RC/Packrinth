...
pub fn summarize_needed(messages: &[AiMessage], max_chars: usize) -> bool {
	messages.iter().map(|message| message.content.len()).sum::<usize>() > max_chars
}
...
/// - 返回被丢弃的消息数。
pub fn enforce_window(messages: &mut Vec<AiMessage>, max_chars: usize) -> usize {
	...
	// 第二遍：剩余预算填充其他消息（按原顺序）。
	for message in messages.iter() {
		if !matches!(message.role, AiMessageRole::Tool) && message.content.len() <= budget {
			middle.push(message.clone());
...
mod tests {
	...
	fn prioritizes_tool_messages() {
		...
		assert!(dropped > 0);
		// tool 消息保留
		assert!(messages.iter().any(|m| matches!(m.role, AiMessageRole::Tool)));
