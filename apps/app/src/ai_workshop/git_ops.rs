...
use git2::{BranchType, IndexAddOption, Patch, Repository, Signature, Status, StatusOptions};
...
fn open_repo(root: &Path) -> Result<Repository, String> {
	Repository::open(root).map_err(|e| format!("无法打开 git 仓库 {}: {e}", root.display()))
}
...
fn git_log_impl(root: &Path, limit: usize) -> Result<Value, String> {
	...
	revwalk.push_head().map_err(|e| format!("仓库没有 HEAD: {e}"))?;
		...
		"list" => {
			...
			for branch in branches {
				...
				let name = branch
					...
					.unwrap_or("")
					.to_string();
				names.push(json!({ "name": name, "current": branch.is_head() }));
			}
...
fn git_diff_impl(root: &Path, commit: Option<&str>) -> Result<Value, String> {
	...
	let base_tree = match commit {
		Some(c) => {
			let obj = repo.revparse_single(c).map_err(|_| format!("找不到 commit: {c}"))?;
	...
	for (i, delta) in diff.deltas().enumerate() {
		...
		if let Some(patch) = Patch::from_diff(&diff, i).map_err(|e| e.to_string())? {
...
impl Tool for GitInitTool {
	fn info(&self) -> ToolInfo {
		ToolInfo {
			...
			description: "在实例根目录初始化 git 仓库（已存在则提示）。".to_string(),
...
impl Tool for GitCheckoutTool {
	fn info(&self) -> ToolInfo {
		ToolInfo {
			...
			description: "检出指定 commit 或分支（非分支时进入 detached HEAD）。".to_string(),
			domain: ToolDomain::Git,
			requires_confirmation: true,
			is_readonly: false,
			params_schema: json!({
...
impl Tool for GitBranchTool {
	fn info(&self) -> ToolInfo {
		ToolInfo {
			...
			description: "管理 git 分支：list | create | delete | checkout。".to_string(),
			domain: ToolDomain::Git,
			requires_confirmation: true,
			is_readonly: false,
			params_schema: json!({
...
impl Tool for GitDiffTool {
	fn info(&self) -> ToolInfo {
		ToolInfo {
			...
			description: "查看 git 变更的增删行统计（缺省为工作区 vs HEAD）。".to_string(),
			domain: ToolDomain::Git,
			requires_confirmation: false,
			is_readonly: true,
			params_schema: json!({
...
/// 构造并注册全部 Git 版本控制工具。
pub fn register_git_ops_tools(registry: &Arc<super::tools::registry::ToolRegistry>) {
