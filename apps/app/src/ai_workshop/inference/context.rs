...
use crate::ai_workshop::providers::provider_trait::{
    AiMessage, AiMessageRole, ToolCall,
};
...
impl InferenceContext {
    ...
    pub async fn build_messages(&self, user_message: &str) -> Vec<AiMessage> {
        ...
        let max_inject =
            self.state.config_manager.config().skills.max_inject_count;
        for skill in self
            .state
            .skill_loader
            .enabled_skills()
    ...
    pub async fn trim(&self, messages: Vec<AiMessage>) -> Vec<AiMessage> {
        ...
        let mut budget = 120_000usize.saturating_sub(
            trimmed
                .first()
