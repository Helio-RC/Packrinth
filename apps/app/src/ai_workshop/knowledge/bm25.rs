...
use tantivy::schema::{
    Field, IndexRecordOption, OwnedValue, STORED, STRING, Schema, TEXT,
    TantivyDocument,
};
...
impl Bm25Index {
    ...
    pub fn search(
        ...
        let query_parser =
            QueryParser::for_index(&self.index, vec![title, content]);
        ...
        for (score, doc_address) in top_docs {
            let doc =
                searcher.doc::<TantivyDocument>(doc_address).map_err(err)?;
...
mod tests {
        ...
        index
            .add_documents(
                ...
                &[
                    doc(
                        "Install Guide",
                        "How to install the launcher and configure the game.",
                    ),
                    doc(
                        "Troubleshooting",
    ...
    fn search_filters_by_source() {
        ...
        index
            .add_documents(
                "skills",
                &[doc("Guide", "install the launcher here")],
            )
            .unwrap();
        index
