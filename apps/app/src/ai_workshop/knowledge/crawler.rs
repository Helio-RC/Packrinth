...
impl UrlFilter {
	pub fn new(allowed_domains: Vec<String>) -> Self {
		let normalized = allowed_domains
			.into_iter()
			.map(|d| d.trim().trim_start_matches("https://").trim_start_matches("http://").to_lowercase())
			.filter(|d| !d.is_empty())
			.collect();
...
pub async fn crawl(url: &str, filter: &UrlFilter, max_chars: usize) -> Result<Vec<SourceDocument>> {
