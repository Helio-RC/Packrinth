...
fn response_take_result(response: &Value) -> std::result::Result<Value, String> {
	...
	response.get("result").cloned().ok_or_else(|| "MCP 响应缺少 result".to_string())
}
...
impl McpClient {
	...
	async fn run_loop(self: std::sync::Arc<Self>, event_rx: mpsc::Receiver<Value>, tool_registry: Arc<ToolRegistry>) {
		let stdin_slot = Arc::new(tokio::sync::Mutex::new(None::<tokio::process::ChildStdin>));
		...
		loop {
			...
			loop {
				...
				.await
				{
					Ok(_) => {}
					Err(e) => {
						tracing::warn!("mcp: health check failed: {e}; restarting mcp process");
						break;
					}
				}
			}
	...
	async fn clear_stdin_slot(&self, slot: &Arc<tokio::sync::Mutex<Option<tokio::process::ChildStdin>>>) {
	...
	async fn sync_tools(&self, registry: &ToolRegistry) {
		...
		for schema in &schemas {
			...
			registry.register(std::sync::Arc::new(McpTool::new(schema, self.shared.clone())));
		}
...
mod tests {
	...
	fn response_result_extracted() {
		let response = json!({ "jsonrpc": "2.0", "id": 1, "result": { "answer": 42 } });
		assert_eq!(response_take_result(&response).unwrap()["answer"], json!(42));
	}
