...
use anthropic_sdk::{
	Anthropic, ContentBlock, ContentBlockDelta, ContentBlockParam, Message, MessageContent,
	MessageStreamEvent, Role, Tool, MessageCreateParams, MessageParam,
};
...
impl AnthropicProvider {
	...
	fn client(&self) -> Result<Anthropic, ProviderError> {
		Anthropic::new(self.api_key.clone()).map_err(|e| ProviderError(e.to_string()))
	}
}
			...
			let event = match chunk {
				...
				MessageStreamEvent::MessageDelta { usage, .. } => StreamEvent {
					...
					usage: Some(AiUsage {
						...
						total_tokens: usage.input_tokens.unwrap_or(0) as u64 + usage.output_tokens as u64,
					}),
					done: false,
					error: None,
				},
...
fn build_params(
	...
	MessageCreateParams {
		...
		stop_sequences: None,
		stream: None,
		tools: (!tools.is_empty()).then(|| tools.iter().map(to_anthropic_tool).collect()),
		tool_choice: None,
		metadata: None,
	}
}
...
fn message_to_ai(message: Message) -> AiResponse {
	...
	let usage = Some(AiUsage {
		...
		total_tokens: (message.usage.input_tokens as u64) + (message.usage.output_tokens as u64),
	});
	AiResponse {
