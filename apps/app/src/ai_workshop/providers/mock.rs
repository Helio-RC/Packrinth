...
impl AiProvider for MockProvider {
    ...
    async fn summarize(
        ...
        Ok(
            "【Mock 摘要】用户请求安装 JEI（已完成搜索），当前等待工具确认。"
                .to_string(),
        )
    }
    ...
    async fn chat(
        ...
        let value: serde_json::Value =
            serde_json::from_str(match_case(messages)).map_err(|e| {
                ProviderError(format!("mock 案例解析失败: {e}"))
            })?;
...
mod tests {
    ...
    async fn multi_install_triggers_two_tool_calls() {
        let provider = MockProvider;
        let response =
            provider.chat(&user_message("安装多个"), &[]).await.unwrap();
    ...
    async fn stream_sends_events_and_terminates() {
        ...
        assert!(
            events.iter().any(|e| e.delta.is_some()),
            "stream should emit deltas"
        );
        assert!(
            events.last().unwrap().done,
