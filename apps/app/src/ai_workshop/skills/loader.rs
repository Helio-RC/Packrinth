...
impl SkillLoader {
    ...
    pub fn force_load(&self, name: &str) -> Result<Skill> {
        self.get_skill(name)
            .ok_or_else(|| other_err(format!("Unknown skill: {name}")))
    }
    ...
    pub async fn import_skill(&self, path: &str) -> Result<()> {
        ...
        let canonical_src = std::fs::canonicalize(&src).map_err(|e| {
            other_err(format!("无法解析来源路径 {}: {e}", src.display()))
        })?;
...
fn load_skill_from_dir(dir: &Path) -> std::result::Result<Skill, String> {
    ...
    let parsed: SkillToml = toml::from_str(&raw)
        .map_err(|e| format!("解析 skill.toml 失败: {e}"))?;
    validate_toml(&parsed)?;

    let guide_path = dir.join("guide.md");
    let guide_md = if guide_path.is_file() {
        let raw_guide = std::fs::read_to_string(&guide_path)
            .map_err(|e| format!("读取 guide.md 失败: {e}"))?;
...
"#,
        ...
        assert!(
            !skills[0].enabled,
            "newly loaded skills must default to disabled"
        );
