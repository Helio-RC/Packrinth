...
pub async fn pack_mods_dir<F>(src: &Path, dest_zip: &Path, progress: F) -> Result<PackStats>
where
...
impl ExecutableToolchain for ExportModsToolchain {
	...
	async fn execute(
		...
		let stats = pack_mods_dir(&mods_dir, &dest_zip, |packed, percent, path| {
			ctx.report_progress(
				"packing",
				Some(percent),
...
mod tests {
	...
	fn temp_dir(tag: &str) -> PathBuf {
		let base = std::env::temp_dir().join(format!("packrinth_{tag}_{}", uuid::Uuid::new_v4()));
	...
	async fn packs_files_with_subdirectories_and_reports_progress() {
		...
		let stats = pack_mods_dir(&src, &dest, |packed, percent, path| {
			progress
				.lock()
				.unwrap()
				.push((packed, percent, path.to_string_lossy().to_string()));
		})
		.await
		.unwrap();
