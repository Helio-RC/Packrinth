...
pub(crate) async fn resolve_write_path(
    ...
    let parent = target.parent().ok_or_else(|| format!("无效路径: {rel}"))?;
...
async fn make_backup(
    ...
    match tokio::fs::symlink_metadata(&target).await {
        ...
        Err(e) => {
            return Err(format!("无法检查目标 {}: {e}", target.display()));
        }
    }
    let parent = target.parent().ok_or_else(|| format!("无效路径: {rel}"))?;
    let file_name = target
    ...
    let backup = parent.join(format!(
        "{}{ts}",
        backup_prefix(&file_name.to_string_lossy())
    ));
    ...
    async fn execute(
        ...
        while let Some((abs, rel)) = stack.pop() {
            ...
            while let Some(entry) =
                rd.next_entry().await.map_err(|e| e.to_string())?
            {
                if files.len() >= 500 {
                    break;
                }
...
impl Tool for DiffConfigTool {
    fn info(&self) -> ToolInfo {
        ToolInfo {
            ...
            description: "对比配置文件当前内容与最新备份，返回逐行差异。"
                .to_string(),
            domain: ToolDomain::Config,
            requires_confirmation: false,
            is_readonly: true,
...
/// 构造并注册全部配置读写工具。
pub fn register_config_ops_tools(
...
mod tests {
    ...
    async fn diff_config_no_backup_returns_error() {
        ...
        let res =
            DiffConfigTool::diff_config_impl(&root, "config/jei.toml").await;
    ...
    async fn canonicalize_within_rejects_symlink_escape() {
        ...
        let res =
            DiffConfigTool::diff_config_impl(&root, "config/evil.toml").await;
    ...
    async fn backup_generate_and_rollback() {
        ...
        // 模拟 rollback 写回
        tokio::fs::write(&target, restored.as_bytes())
            .await
            .unwrap();
