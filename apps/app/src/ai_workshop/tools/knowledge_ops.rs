...
impl Default for CrawlDocumentTool {
	fn default() -> Self {
		Self::new(DEFAULT_ALLOWED_DOMAINS.iter().map(|d| d.to_string()).collect())
	}
}
...
impl Tool for CrawlDocumentTool {
	fn info(&self) -> ToolInfo {
		ToolInfo {
			...
			description: "抓取网页正文并分块，供检索外部文档（仅限白名单域名）。".to_string(),
	...
	async fn execute(
		...
		let docs = crate::ai_workshop::knowledge::crawler::crawl(&url, &filter, max_chars)
			.await
...
mod tests {
	...
	async fn crawl_document_rejects_non_whitelisted_domain_without_network() {
		...
		let result = tool
			.execute(json!({ "url": "https://example.com/docs" }), &ExecutionContext::default())
			.await;
	...
	async fn crawl_document_rejects_invalid_url_without_network() {
		...
		let result = tool
			.execute(json!({ "url": "not-a-url" }), &ExecutionContext::default())
			.await;
		assert!(result.is_err());
	}
