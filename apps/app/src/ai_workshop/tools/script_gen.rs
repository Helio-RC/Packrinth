...
impl Tool for GenerateKubejsScriptTool {
	fn info(&self) -> ToolInfo {
		ToolInfo {
			...
			description: "生成并写入一个 KubeJS 脚本到实例的 kubejs 目录。".to_string(),
...
impl Tool for GenerateCrafttweakerScriptTool {
	fn info(&self) -> ToolInfo {
		ToolInfo {
			...
			description: "生成并写入一个 CraftTweaker 脚本（.zs）到实例的 scripts 目录。".to_string(),
			domain: ToolDomain::Script,
			requires_confirmation: true,
			is_readonly: false,
			params_schema: json!({
...
/// 构造并注册全部脚本生成工具。
pub fn register_script_gen_tools(registry: &Arc<super::registry::ToolRegistry>) {
