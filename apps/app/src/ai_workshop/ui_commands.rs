...
pub async fn execute_tool<R: Runtime>(
    ...
    execute_tool_with_timeout(state, app, task_id, name, params, TOOL_TIMEOUT)
        .await
}
...
pub async fn execute_toolchain(
    ...
    tokio::time::timeout(
        TOOL_TIMEOUT,
        toolchain.execute(instance_id.as_deref(), params, &context),
    )
    .await
...
async fn execute_tool_with_timeout<R: Runtime>(
    ...
    match tokio::time::timeout(timeout, tool.execute(params, &context)).await {
        ...
        Ok(Err(e)) => {
            ...
            state.task_registry.remove(task_id);
            Ok(
                serde_json::json!({ "success": false, "error": { "code": "TOOL_ERROR", "message": e } }),
            )
        }
        Err(_) => {
            ...
            state.task_registry.remove(task_id);
            Ok(
                serde_json::json!({ "success": false, "error": { "code": "TOOL_TIMEOUT", "message": "工具执行超时" } }),
            )
        }
    }
}
...
mod tests {
    ...
    use crate::ai_workshop::tools::context::{
        InstanceLockManager, TaskRegistry,
    };
    ...
    use crate::ai_workshop::{
        AiWorkshopState, config::AiWorkshopConfig, config::ConfigManager,
    };
    ...
    fn test_state() -> AiWorkshopState {
        ...
        let dir = std::env::temp_dir()
            .join(format!("packrinth-ai-uicmd-{nanos}-{seq}"));
